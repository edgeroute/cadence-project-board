...
async function runWalkGrep(pattern, root, signal) {
  ...
  const push = (line) => {
    ...
    if (budget < 0) {
      hits2.push(`[output truncated at ${GREP_OUTPUT_LIMIT} bytes]`);
      return false;
    }
    hits2.push(line);
    return true;
...
function transformOutputFormat(params) {
  ...
  const { output_format, ...rest2 } = params;
  return {
    ...rest2,
    output_config: {
...
var init_client = __esm({
  "node_modules/@anthropic-ai/sdk/client.mjs"() {
    ...
    BaseAnthropic = class {
      ...
      buildURL(path7, query2, defaultBaseURL) {
      ...
      async buildRequest(inputOptions, { retryCount = 0 } = {}) {
        ...
        const { method, path: path7, query: query2, defaultBaseURL } = options;
        if (this._authState.resolution) {
          await this._authState.resolution;
        }
        ...
        const url = this.buildURL(path7, query2, defaultBaseURL);
...
var HIGH_STEM = /\b(crash|vulnerab|regress|corrupt|leak|block)/i;
var HIGH_WORD = /\b(data.?loss|security|broken|silent|urgent|blocker)\b/i;
var LOW_STEM = /\b(cosmetic|polish)/i;
var LOW_WORD = /\b(nit|nitpick|typo|cleanup|docs?|comment|rename|wording)\b/i;
var BIG_STEM = /\b(migrat|rewrit|refactor|architect|redesign|overhaul)/i;
var BIG_WORD = /\b(epic|tracking|umbrella)\b/i;
var SMALL_STEM = /\b(typo|renam)/i;
var SMALL_WORD = /\b(bump|one.?line|constant|copy|wording|label)\b/i;
var HIGH_SIGNALS = { stem: HIGH_STEM, word: HIGH_WORD };
var LOW_SIGNALS = { stem: LOW_STEM, word: LOW_WORD };
var BIG_SIGNALS = { stem: BIG_STEM, word: BIG_WORD };
var SMALL_SIGNALS = { stem: SMALL_STEM, word: SMALL_WORD };
function hits(signals, text) {
  return signals.stem.test(text) || signals.word.test(text);
}
function heuristicFor(brief, priorities, sizes) {
...
var MAX_TOKENS = 16e3;
var SYSTEM = `You triage a software project's backlog.
...
async function prioritize(board, items, apiKey, model) {
  ...
  const suggestions = briefs.map((b, i) => {
  ...
  try {
    ...
    return {
      suggestions,
      source: "heuristic",
      note: `Claude couldn't be reached (${e.message}) \u2014 these are keyword-scored instead.`
    };
  }
}
...
var CONFIG_FILE = import_path28.default.join(CONFIG_DIR, "project-board.json");
...
async function readConfig(projectPath) {
  ...
  let parsed = null;
  try {
    const raw = await import_fs.promises.readFile(import_path28.default.join(projectPath, CONFIG_FILE), "utf8");
...
async function ensureGitignored(projectPath) {
  const gitignore = import_path28.default.join(projectPath, ".gitignore");
...
async function writeConfig(projectPath, input) {
  ...
  const dir = import_path28.default.join(projectPath, CONFIG_DIR);
  await import_fs.promises.mkdir(dir, { recursive: true });
  let existing = null;
  try {
    existing = JSON.parse(await import_fs.promises.readFile(import_path28.default.join(projectPath, CONFIG_FILE), "utf8"));
  } catch {
    existing = null;
  }
  const token = input.token?.trim() || existing?.token || void 0;
  const anthropicKey = input.anthropicKey?.trim() || existing?.anthropicKey || void 0;
  const aiModel = input.aiModel?.trim() || existing?.aiModel || void 0;
  const body = {
  ...
  await import_fs.promises.writeFile(import_path28.default.join(projectPath, CONFIG_FILE), JSON.stringify(body, null, 2) + "\n", {
    encoding: "utf8",
...
// src/backend/label.service.ts
var REST2 = "https://api.github.com";
var COLOURS = {
...
var DEFAULT_COLOUR = "c5def5";
function labelNameFor(fieldName, optionName) {
  return `${fieldName.toLowerCase()}:${optionName}`;
}
function parseRepo(nameWithOwner) {
  const [owner, repo] = nameWithOwner.split("/");
  return owner && repo ? { owner, repo } : null;
}
async function rest(config, method, path7, body) {
  const res = await fetch(`${REST2}${path7}`, {
    method,
    headers: {
...
async function ensureLabel(config, ref, name, optionName) {
  ...
  const created = await rest(config, "POST", path7, {
    name,
...
function installSkill() {
  try {
    ...
    if (!home) return;
    const source = import_path29.default.join(__dirname, "..", "skill", "SKILL.md");
    if (!import_fs2.default.existsSync(source)) return;
    const wanted = import_fs2.default.readFileSync(source, "utf8");
    const target = import_path29.default.join(home, ".claude", "skills", "board", "SKILL.md");
    const current = import_fs2.default.existsSync(target) ? import_fs2.default.readFileSync(target, "utf8") : null;
    if (current === wanted) return;
    import_fs2.default.mkdirSync(import_path29.default.dirname(target), { recursive: true });
...
async function handlePrioritize(req, res) {
  ...
  sendJson(res, 200, await prioritize(board, items, config.anthropicKey ?? null, config.aiModel));
}
